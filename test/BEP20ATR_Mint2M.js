...
/*
*  minting tests for accounts that have a cliff of 2 months
*/

it('Test5 : try to mint for  ICO Round 2 befor 2M lockup at 15 mars 2022 00:00:00 GMT+01:00', async () => {
  const bep = await BEP20.deployed();
...
it('Test6 : try to mint for  ICO Round 2 after 2 M lockup at 15 avril 2022 00:00:01 GMT+02:00', async () => {
  const bep = await BEP20.deployed();
  await time.increaseTo(1649973601);
  await bep.mint(2);
  const balanceAss  = await bep.balanceOf(accounts[1]);
  const balanceTeam = await bep.balanceOf(accounts[2]);
  const balanceSeed = await bep.balanceOf(accounts[3]);
  ...
  const balanceICOR3 = await bep.balanceOf(accounts[8]);
  const balanceReserve = await bep.balanceOf(accounts[9]);
  assert.equal(balanceICOR2.valueOf(), (60000000000000000 + 60000000000000000), "The Ballance should be  60000000000000000  + 60000000000000000");
  assert.equal(balanceReserve.valueOf(), 79200000000000000, "The Ballance should be  79200000000000000");
  assert.equal(balanceICOR3.valueOf(),   450000000000000000, "The Ballance should be  450000000000000000");
  assert.equal(balanceICOR1.valueOf(), 22500000000000000, "The Ballance should be  22500000000000000");
  assert.equal(balancePriv.valueOf(),  1998000000000000 , "The Ballance should be  1998000000000000");
...
it('Test7 : try to mint for  ICO Round 2 after 2 days  after  2 M lockup at 17 avril 2022 00:00:01 GMT+02:00', async () => {
  const bep = await BEP20.deployed();
  await time.increaseTo(1650146401);
  await bep.mint(2);
  const balanceAss  = await bep.balanceOf(accounts[1]);
  const balanceTeam = await bep.balanceOf(accounts[2]);
  const balanceSeed = await bep.balanceOf(accounts[3]);
...
it('Test8 : try to mint for  ICO Round 2 after 30 days  after  2 M lockup at 15 mai 2022 00:00:01 GMT+02:00', async () => {
  const bep = await BEP20.deployed();
  ...
  const balanceICOR3 = await bep.balanceOf(accounts[8]);
  const balanceReserve = await bep.balanceOf(accounts[9]);
  assert.equal(balanceICOR2.valueOf(), (60000000000000000 + 2*60000000000000000), "The Ballance should be  60000000000000000+ 2*60000000000000000");
  assert.equal(balanceReserve.valueOf(), 79200000000000000, "The Ballance should be  79200000000000000");
  assert.equal(balanceICOR3.valueOf(),   450000000000000000, "The Ballance should be  450000000000000000");
  assert.equal(balanceICOR1.valueOf(), 22500000000000000, "The Ballance should be  22500000000000000");
  assert.equal(balancePriv.valueOf(),  1998000000000000 , "The Ballance should be  1998000000000000");
...
it('Test9 : try to mint for ICO Round 2 every 30 days  after  2 M lockup starting at  15 juin 2022 00:00:01 GMT+02:00 ( 2 Mint)', async () => {
  const bep = await BEP20.deployed();
  for (let step = 0; step < 2; step++) {
    await time.increaseTo(1655244001  + step*2674800);
    await bep.mint(2);
    const balanceAss  = await bep.balanceOf(accounts[1]);
    const balanceTeam = await bep.balanceOf(accounts[2]);
    const balanceSeed = await bep.balanceOf(accounts[3]);
    ...
    const balanceICOR3 = await bep.balanceOf(accounts[8]);
    const balanceReserve = await bep.balanceOf(accounts[9]);
    assert.equal(balanceICOR2.valueOf(), (60000000000000000 + (3+step)*60000000000000000), "The Ballance should be  60000000000000000  + (3+step)*60000000000000000");
    assert.equal(balanceReserve.valueOf(), 79200000000000000, "The Ballance should be  79200000000000000");
    assert.equal(balanceICOR3.valueOf(),   450000000000000000, "The Ballance should be  450000000000000000");
    assert.equal(balanceICOR1.valueOf(), 22500000000000000, "The Ballance should be  22500000000000000");
    assert.equal(balancePriv.valueOf(),  1998000000000000 , "The Ballance should be  1998000000000000");
    ...
    const totals = balanceICOR2 + "  " ;
    console.log("totals - "+ step  +" : "+totals);

  }
});

it('Test10 : try to mint for  ICO Round 2 after 6 M (2 M + 4 M) at 15 novembre 2022 00:00:01 GMT+01:00', async () => {
  const bep = await BEP20.deployed();
  await time.increaseTo(1668466801);
  await bep.mint(2);
  const balanceICOR2 = await bep.balanceOf(accounts[7]);
  const totals = balanceICOR2 + "  " ;
    console.log("totals - "+ 3  +" : "+totals);
});
