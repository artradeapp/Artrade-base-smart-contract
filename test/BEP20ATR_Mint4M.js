...
contract('BEP20', (accounts) => {
  ...
  /*
  *  minting tests for accounts that have a cliff of 4 months
  */


  it('Test5 : try to mint for  Private Sale and Marketing befor 4M lockup at 15 mars 2022 00:00:00 GMT+01:00', async () => {
    const bep = await BEP20.deployed();
  ...
  it('Test6 : try to mint for  Private Sale and Marketing after 4 M lockup at 15 juin 2022 00:00:01 GMT+02:00', async () => {
    const bep = await BEP20.deployed();
    ...
    const balanceICOR3 = await bep.balanceOf(accounts[8]);
    const balanceReserve = await bep.balanceOf(accounts[9]);
    assert.equal(balancePriv.valueOf(),  (1998000000000000 + 1498500000000000), "The Ballance should be  1998000000000000 + 1498500000000000");
    assert.equal(balanceMark.valueOf(),  (7200000000000000 + 5400000000000000), "The Ballance should be  7200000000000000 + 5400000000000000");
    assert.equal(balanceReserve.valueOf(), 79200000000000000, "The Ballance should be  79200000000000000");
    assert.equal(balanceICOR3.valueOf(),   450000000000000000, "The Ballance should be  450000000000000000");
    assert.equal(balanceICOR2.valueOf(), 60000000000000000, "The Ballance should be  60000000000000000");
    assert.equal(balanceICOR1.valueOf(), 22500000000000000, "The Ballance should be  22500000000000000");
  ...
  it('Test7 : try to mint for  Private Sale and Marketing after 2 days  after  4 M lockup at 17 juin 2022 00:00:01 GMT+02:00', async () => {
    const bep = await BEP20.deployed();
    await time.increaseTo(1655416801);
    await bep.mint(4);
    const balanceAss  = await bep.balanceOf(accounts[1]);
    const balanceTeam = await bep.balanceOf(accounts[2]);
    const balanceSeed = await bep.balanceOf(accounts[3]);
  ...
  it('Test8 : try to mint for  Private Sale and Marketing after 30 days  after  4 M lockup at 15 juillet 2022 00:00:01 GMT+02:00', async () => {
    const bep = await BEP20.deployed();
    await time.increaseTo(1657836001);
    await bep.mint(4);
    const balanceAss  = await bep.balanceOf(accounts[1]);
    const balanceTeam = await bep.balanceOf(accounts[2]);
    const balanceSeed = await bep.balanceOf(accounts[3]);
    ...
    const balanceICOR3 = await bep.balanceOf(accounts[8]);
    const balanceReserve = await bep.balanceOf(accounts[9]);
    assert.equal(balancePriv.valueOf(),  (1998000000000000 + 2*1498500000000000), "The Ballance should be  1998000000000000 + 2*1498500000000000");
    assert.equal(balanceMark.valueOf(),  (7200000000000000 + 2*5400000000000000), "The Ballance should be  7200000000000000+ 2*5400000000000000 ");
    assert.equal(balanceReserve.valueOf(), 79200000000000000, "The Ballance should be  79200000000000000");
    assert.equal(balanceICOR3.valueOf(),   450000000000000000, "The Ballance should be  450000000000000000");
    assert.equal(balanceICOR2.valueOf(), 60000000000000000, "The Ballance should be  60000000000000000");
    assert.equal(balanceICOR1.valueOf(), 22500000000000000, "The Ballance should be  22500000000000000");
  ...
  it('Test9 : try to mint for  Private Sale and Marketing every 30 days  after  4 M lockup starting from  15 août 2022 00:00:01 GMT+02:00 ( 10 Mint)', async () => {
    const bep = await BEP20.deployed();
    for (let step = 0; step < 10; step++) {
      await time.increaseTo(1660514401  + step*2674800);
      await bep.mint(4);
      const balanceAss  = await bep.balanceOf(accounts[1]);
      const balanceTeam = await bep.balanceOf(accounts[2]);
      const balanceSeed = await bep.balanceOf(accounts[3]);
      ...
      const balanceICOR3 = await bep.balanceOf(accounts[8]);
      const balanceReserve = await bep.balanceOf(accounts[9]);
      assert.equal(balancePriv.valueOf(),  (1998000000000000 + (3+step)*1498500000000000), "The Ballance should be  1998000000000000 + (3+step)*1498500000000000");
      assert.equal(balanceMark.valueOf(),  (7200000000000000 + (3+step)*5400000000000000), "The Ballance should be  7200000000000000 + (3+step)*5400000000000000");
      assert.equal(balanceReserve.valueOf(), 79200000000000000, "The Ballance should be  79200000000000000");
      assert.equal(balanceICOR3.valueOf(),   450000000000000000, "The Ballance should be  450000000000000000");
      assert.equal(balanceICOR2.valueOf(), 60000000000000000, "The Ballance should be  60000000000000000");
      assert.equal(balanceICOR1.valueOf(), 22500000000000000, "The Ballance should be  22500000000000000");
      ...
      const totals = balancePriv + "  "+ balanceMark ;
      console.log("totals - "+ step  +" : "+totals);

    }
  });

  it('Test10 : try to mint for  Private Sale and Marketing after 16 M (4 M + 12 M) at 15 juin 2023 00:00:01 GMT+02:00', async () => {
    const bep = await BEP20.deployed();
    await time.increaseTo(1686780001);
    await bep.mint(4);
  });
